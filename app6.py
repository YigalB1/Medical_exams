    ...
    for page_idx in range(doc.page_count):
        ...
        for block in blocks:
            ...
            if re.match(r"^\d\s*\.", first_line):
                ...
                q_num = int(m.group(1))
            # 2-digit questions: "10"  "11"  (number alone, no dot)
            elif re.match(r"^\d{2}\s*\.?\s*$", first_line):
                m = re.match(r"^(\d{2})", first_line)
                q_num = int(m.group(1))
            # 2-digit questions with dot+text: "10. text..."
            elif re.match(r"^\d{2}\s*\.\s+\S", first_line):
                m = re.match(r"^(\d{2})", first_line)
                q_num = int(m.group(1))
            else:
                continue
